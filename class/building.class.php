...
class Building extends database_object { 
  ...
  public static function build_cache($objects) {
    ...
    $sql = 'SELECT * FROM `building` WHERE `building`.`uid` IN ' . $idlist; 
    $db_results = Dba::read($sql); 

    while ($row = Dba::fetch_assoc($db_results)) { 
      parent::add_to_cache('building',$row['uid'],$row); 
    }
	...
	public function refresh() { 
		...
		Building::remove_from_cache('building',$this->uid); 
   ...
   */
    ...
    if ($enabled === true) {
      ...
      $enabled_sql = "1=1";
		...
		$sql = "SELECT * FROM `building` WHERE $enabled_sql"; 
		$db_results = Dba::read($sql); 

		$results = array(); 
		while ($row = Dba::fetch_assoc($db_results)) { 
      parent::add_to_cache('building',$row['uid'],$row); 
			$results[] = new Building($row['uid']); 
		} 
   ...
   */
	public static function name_to_id($name) { 
		...
		$sql = "SELECT `uid` FROM `building` WHERE `name` LIKE '$name'"; 
  ...
  public function enable() { 
    ...
    $sql = "UPDATE `building` SET `enabled`='1' WHERE `uid`='$uid'";
  ...
  public function disable() { 
    ...
    $uid = Dba::escape($this->uid);
    $sql = "UPDATE `building` SET `enabled`='0' WHERE `uid`='$uid'";
    $db_results = Dba::write($sql);
  ...
  /**
   * create
   * This is used for creating a new building
   */
  public static function create($input) { 
    ...
    if (Building::name_to_id($input['name'])) { 
      Err::add('general','Duplicate Building - name already exists');
      return false;
    }
    ...
    if (!$institution->enabled()) { 
      Err::add('general','Institution not enabled');
      return false;
    }

    // Nothing else to check... yet
    $sql = "INSERT INTO `building` SET `name`=?, `institution`=?, `enabled`='0'";
