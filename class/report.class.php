...
class Report {
  ...
  private function csv_siterecord($site) { 
    ...
    $header = array('site','accession','catalog id','excavation count','unit','level','litho unit','station index','xrf matrix index','weight','height','width','thickness','quantity','material','classification','quad','feature','krotovina','notes','created','northing','easting','elevation','user');
    ...
    foreach ($results as $record_uid) {
      ...
      $record_data = array($site->name,$site->accession,$record->catalog_id,$record->excavation_count,$record->level->unit->name,$record->level->record,$record->lsg_unit->name,
