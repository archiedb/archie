...
class Room extends database_object { 
  ...
  public static function build_cache($objects) {
    ...
    $sql = 'SELECT * FROM `room` WHERE `room`.`uid` IN ' . $idlist; 
    $db_results = Dba::read($sql); 

    while ($row = Dba::fetch_assoc($db_results)) { 
      parent::add_to_cache('room',$row['uid'],$row); 
    }
	...
	public function refresh() { 
		...
		Room::remove_from_cache('room',$this->uid); 
   ...
   */
    ...
    if ($enabled === true) {
      ...
      $enabled_sql = "1=1";
		...
		$sql = "SELECT * FROM `room` WHERE $enabled_sql"; 
		$db_results = Dba::read($sql); 

		$results = array(); 
		while ($row = Dba::fetch_assoc($db_results)) { 
      parent::add_to_cache('room',$row['uid'],$row); 
			$results[] = new Room($row['uid']); 
		} 
   ...
   */
	public static function name_to_id($name) { 
		...
		$sql = "SELECT `uid` FROM `room` WHERE `name` LIKE '$name'"; 
  ...
  public function enable() { 
    ...
    $sql = "UPDATE `room` SET `enabled`='1' WHERE `uid`='$uid'";
  ...
  public function disable() { 
    ...
    $uid = Dba::escape($this->uid);
    $sql = "UPDATE `room` SET `enabled`='0' WHERE `uid`='$uid'";
    $db_results = Dba::write($sql);
  ...
  /**
   * create
   * This is used for creating a new room
   */
  public static function create($input) { 
    ...
    if (Room::name_to_id($input['name'])) { 
      Err::add('general','Duplicate Room - name already exists');
      return false;
    }
    ...
    $sql = "INSERT INTO `room` SET `name`=?, `building`=?, `enabled`='0'";
