...
$images = Content::krotovina($krotovina->uid,'3dmodel'); 
$i=0; 
?>
<div class="row">
...
foreach ($images as $uid) {
  $model = new Content($uid,'3dmodel','krotovina'); 
  if ($i/3 == floor($i/3)) {
    ...
    <div class="thumbnail">
      ...
      <div class="caption">
        ...
        <?php if (Access::has('media','read')) { ?>
          <a class="btn btn-info btn-small" href="<?php echo Config::get('web_path'); ?>/media/3dmodel/krotovina/<?php $model->_print('uid'); ?>" title="Download">Download</a>
        <?php } ?>
        ...
        if (Access::has('media','delete')) { 
          require \UI\template('/krotovina/modal_delete_3dmodel'); 
        } 
        if (Access::has('media','edit')) { 
          require \UI\template('/krotovina/modal_edit_3dmodel'); 
        } 
        ?>
        <?php } ?>
        </p>
